...
public class observationResourceProvider implements IResourceProvider {
    ...
    public observationResourceProvider() {
    	...
    	//Globales.myPatients = new HashMap<String, Patient>();// Inicializar myPatients
    	myNextId = 1;  	  	
	}
    
    public observationResourceProvider(globals globales) {
    	Globales = globales;    	
    ...
    public MethodOutcome create(@ResourceParam Observation theObservation) {
        // Verificar que el paciente asociado tenga un ID v�lido (Practica 4 parte 2 punto 5)
        String patientId = theObservation.getSubject().getReferenceElement().getIdPart();
        if (!Globales.myPatients.containsKey(patientId)) {
